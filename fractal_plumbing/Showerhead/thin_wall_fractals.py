...
tiers = 1
tier_0_aspect_ratio = 1.0
final_tier_aspect_ratio = 1.0
aspect_ratio_exponent = 0.6
auto_strength_ratio = 1.0
centroid_initialization = "double_circle"
lofting_exponent = 1.5
...
class k_mean_point_class:
    def __init__(self, incoming_point, incoming_weight = 1.0, incoming_direction_vector = universal_direction_vector):
...
def point_list_2_k_mean_point_object(incoming_point_list, incoming_weights, incoming_direction_vectors = None):
    k_mean_object_list = []
...
class curve_class:
    ...
    def populate_curve_with_circle_info(self, incoming_total_weight, incoming_total_area, incoming_lofting_exponent):
        ...
        target_start_surface_area = (self.centroid.combined_groups_weights/incoming_total_weight) * incoming_total_area
        ...
        target_end_surface_area = (self.k_means_point.weight/incoming_total_weight) * incoming_total_area
...
outlet_circle_radius = .041
connector_height = .12
wall_thickness = .002
outlet_point_circle = create_circle(origin_3d, z_axis, outlet_circle_radius)
starting_injector_points= points_along_circle(outlet_point_circle, 3, False)

#number_of_rings = len(points_per_ring)
#starting_injector_points = []
#starting_injector_points.append(Point.Create(.01336, .006, 0))
#starting_injector_points.append(Point.Create(.006, -.01336, 0))
#starting_injector_points.append(Point.Create(-.006, .01336, 0))
#starting_injector_points.append(Point.Create(-.01336, -.006, 0))
#starting_injector_points.append(Point.Create(.00765, .01848, 0))

starting_weights = [1.0] * len(starting_injector_points)
total_weights = sum(starting_weights)
combined_outlet_surface_area = math.pi*outlet_radius*outlet_radius*len(starting_injector_points)
#-----------------------------------------------------------------------------------------------------------------------------


#---------------------------------------------------Geometry Creation-----------------------------------------------------------
sc_parent_point = Point.Create(0,0,connector_height)
k_means_points = []
master_curves = []

for iterator_2 in starting_injector_points:
    transfer_object = k_mean_point_class(iterator_2, 1.0, universal_direction_vector)
    k_means_points.append(transfer_object)

centroid_point = centroid_class(sc_parent_point, 0, universal_direction_vector)
centroid_point.combined_groups_weights = total_weights

for iterator_2 in k_means_points:
    (transfer_curve_segment, transfer_design_curve) = hermite_curve_auto_strength(centroid_point.current_point, centroid_point.direction_vector, iterator_2.point, iterator_2.direction_vector, auto_strength_ratio)
    master_curves.append(curve_class(transfer_curve_segment, transfer_design_curve, iterator_2, centroid_point))

#Populating the curves with circles and lofting them
for iterator_4 in range(len(master_curves)):
    master_curves[iterator_4].populate_curve_with_circle_info(total_weights, combined_outlet_surface_area, lofting_exponent)
    master_curves[iterator_4].populate_curve_with_circles()
    master_curves[iterator_4].loft_circle_surfaces()

fluid_comp = ComponentHelper.CreateAtRoot("fluids")
ComponentHelper.SetActive(fluid_comp)

for iterator_4 in range(len(master_curves)):
    master_curves[iterator_4].populate_curve_with_circles_with_added_thickness(wall_thickness)
    master_curves[iterator_4].loft_circle_surfaces()



"""
Set 1: Quadrent split
Parent = [0,0,.01465]
Children = [.01336, .006, 0], [.006, -.01336, 0], [-.006, .01336, 0], [-.01336, -.006, 0]

Set 2: Linear split of 5
Parent = [.0136, .0136, .02154]
Children = [.01848,.00765, 0], [.01663, .01111, 0], [.01414, .01414, 0], [.01111, .01663, 0], [.00765, .01848, 0]
