...
inner_radius_bean = .003
start_circle_area = .0000219911

# inner_radius_bean = .0025
# start_circle_area = .0000219911 * 2.00
#-------------------------------------------------------------Functions-----------------------------------------------------------------
...
def create_bean(bean_center, bean_tangent_direction, inner_rad_bean, bean_area, arc_length_degrees, move_center = None):
    #Calculate how large the offset should be to hit the desired area
    a = ((arc_length_degrees/360.0) +0.25)
    b = (2*inner_rad_bean*arc_length_degrees)/360.0
    c = -bean_area/math.pi 
    transfer_value = solve_quadratic(a,b,c)
    inner_outer_offset = transfer_value[0]
    middle_in_out = (inner_outer_offset+inner_rad_bean)/2.0

    #Run a check to see if the ends of the bean will start colliding. If so the offset is incorrect.
    radius_middle = inner_rad_bean + inner_outer_offset/2.0
    distance_from_axis = radius_middle*math.sin(math.radians(180.0-arc_length_degrees/2.0))
    if (distance_from_axis <= inner_outer_offset/2.0) and (arc_length_degrees>180):
        a = 1
        b = 2*inner_rad_bean
        c = -bean_area/math.pi
        transfer_value = solve_quadratic(a,b,c)
    ...
    #All want the centerpoint of each bean to line up. This move lines up the connecting line centerpoint.
    if (move_center != None):
        selection = Selection.Create(piece_1)
        direction = Direction.DirX
        options = MoveOptions()
        result = Move.Translate(selection, direction, -1*(middle_in_out-move_center), options)
    ...
    Delete.Execute(sel)
    return (piece_1, piece_2, middle_in_out)
...
def annulus_2_circle(shape_tangent_direction, target_area, transformation_height, incoming_inner_radius_bean):
    start_degree = 360
    end_degree = 0
    number_of_steps = 15
    degree_step_size = (start_degree-end_degree)/float(number_of_steps)
    height_step = transformation_height/float(number_of_steps)

    shape_list = []
    sel_1 = Selection.Empty()
    sel_2 = Selection.Empty()
    for iterator_13 in range(number_of_steps+1):
        temp_point = Point.Create(0,0, height_step*iterator_13)
        if iterator_13 == 0:
            (half_1, half_2, ideal_center) = create_bean(temp_point, z_axis, inner_radius_bean, start_circle_area, start_degree)
        else :
            (half_1, half_2, not_used) = create_bean(temp_point, z_axis, inner_radius_bean, start_circle_area, start_degree, ideal_center)
        sel_1 = sel_1 + Selection.Create(half_1.Faces[0])
        sel_2 = sel_2 + Selection.Create(half_2.Faces[0])
        start_degree = start_degree - degree_step_size 
...
annulus_2_circle(z_axis, start_circle_area, .015, inner_radius_bean)
...
#     (half_1, half_2) = create_bean(temp_point, z_axis, inner_radius_bean, start_circle_area, start_degree)
#     sel_1 = sel_1 + Selection.Create(half_1.Faces[0])
#     sel_2 = sel_2 + Selection.Create(half_2.Faces[0])
