...
initial_radius = Parameters.initial_radius #Initial radius of inlet that will be split
tiers = Parameters.tiers #How many layers of splits 
xyspread = Parameters.xyspread #This describes how far in the xy direction each set of points splits. The larger the xy spread the more area it will take up
distance_between_tiers = Parameters.distance_between_tiers #This describes how far in the z direction the curve goes before splitting at each layer
fractal_splits = Parameters.fractal_splits
radial_divisor = Parameters.radial_divisor
vector_strength = Parameters.vector_strength
constant_area_interval = 0.005
...
entire_body_volume = Selection.Create(result.CreatedBodies[0])
entire_body_volume.CreateAGroup("Volume")




#Power Selection into named Faces(Works)
sel1 = PowerSelection.Faces.ByArea(cross_sectional_area, 
...
#Power Selection into named Faces(Works)
sel2 = PowerSelection.Faces.ByArea(reductive_cross_sectional_area, 
	PowerSelectOptions(True), 
	SearchCriteria.SizeComparison.Equal)
sel2.SetActive()
happy = Selection.GetActive()
...
difference = sel - sel1 - sel2
difference.CreateAGroup("walls")
"""
