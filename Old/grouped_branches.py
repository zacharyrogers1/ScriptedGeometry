...
initial_radius = .015 #Initial radius of inlet that will be split
tiers = 3 #How many layers of splits 
xyspread = .020 #This describes how far in the xy direction each set of points splits. The larger the xy spread the more area it will take up
distance_between_tiers = .060 #This describes how far in the z direction the curve goes before splitting at each layer
fractal_splits = 4
radial_divisor = 4
vector_strength = 0.09
constant_area_interval = .005
#file_save_location = r""
#-----------------------------------------------------------------------------------------------------------------------------
...
#evaluation_points: You can add as many points as an input as you would like such as 2,3,4, or 5 points.(List of SpaceClaim point objects)
	...
	upper_limit_radius = (target_area/math.pi)**0.5
...
#PoA 3: Run a while loop that creates overlapping circles, check their combined surface area against a reference and increase or decrease the radius until the desired cross-sectional area is achieved.
	...
	while True:	
		...
		#PoA 3.4: Use a linear approximation for the overlapping circles radius  v. Area plot to make the first guess. The first guess happens at iteration = 2.
		if (iteration==0):
			radius_of_circle = (upper_limit_radius + lower_limit_radius)/2
...
#Window.ActiveWindow.Document.SaveAs(file_save_location)
